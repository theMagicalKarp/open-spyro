...
#      (the .ifndef guard sectionize_text.py wrapped each function in), so the
#      symbol isn't doubly defined and its slot is empty for the C to fill;
#   2. the .c is compiled to build/main/c/<Name>.o, which gen_slots_ld.py has
...
# Mirror the auto-generated override manifest the reference keeps, for inspection.
mkdir -p src/asm
{
  echo "# auto-generated by tools/build_main.sh — sets HAVE_C_<sym> for C-overridden funcs"
  for c in "${C_SRCS[@]}"; do echo "HAVE_C += $(basename "$c" .c)"; done
} > src/asm/overrides.mk
