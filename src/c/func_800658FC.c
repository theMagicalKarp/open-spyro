...
/* libcd CD-read interrupt state machine (0x800658FC..0x80065DBC, 1216 bytes).
   One .text section holding four functions (§A row 40): a small setter, the
   root interrupt dispatcher installed by CdInitDriver via libapi slot 2, the
   per-read completion handler, and the streaming-mode per-sector callback.
...
extern int CdGetSector2(int buf, unsigned int nword);
extern int CdPosToInt(unsigned char *loc);
extern int WriteString(char *s);
extern int VSync(int mode);
extern int CdReadStart(int mode);
extern int CdSyncCallback(int fn);
extern int CdReadyCallback(int fn);
extern int CdControlForce(int com, void *param);
extern void FUN_80064050();
extern void FUN_80064074(int arg);
...
extern void (*D_80075144)();    /* read-data callback (CdReadCallback) */
extern volatile int D_80075150; /* read dst pointer */
extern volatile int D_80075158; /* words per sector */
extern volatile int D_8007515C; /* sectors remaining */
...
extern char D_80011F10[]; /* "CdRead: position error" */

void func_80065CC0(void);
...
void func_800659F0(int arg0, int arg1) {
  ...
  if ((arg0 & 0xFF) == 1) {
    if (D_8007515C > 0) {
      if (D_80075158 == 0x200) {
        if (D_80075178 & 1) {
          FUN_80064050(0);
          CdGetSector2((int)loc, 3);
          FUN_80064074(0);
          FUN_80064050(&func_80065CC0);
        } else {
  ...
  if (VSync(-1) > (D_80075164 + 0x4B0)) {
    D_8007515C = -1;
  }
  if ((D_8007515C == 0) || (VSync(-1) > (D_80075164 + 0x4B0))) {
