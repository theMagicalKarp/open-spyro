...
/* Set the type byte (+0x48) to `a1` for every actor in the pickup group named
   by actor `a0`'s +0x43 group index whose current type byte is < 0x80 and
   equal to `a2` (0x8003b538). Out-of-range group index applies the same guarded
   write to the passed actor itself. */
void func_8003B538(int a0, int a1, int a2) {
  unsigned short *ptr;
  int idx;
  int group;
  unsigned int type;
