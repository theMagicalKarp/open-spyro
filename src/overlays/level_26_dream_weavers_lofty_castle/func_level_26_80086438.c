/* func_level_26_80086438 (0x80086438, level_26_dream_weavers_lofty_castle
 ...
 * This variant's classes: 0x0, 0x1, 0x2, 0x6, 0x9, 0xA, 0xC, 0x10, 0x11, 0x15, 0x18, 0x1C, 0x21, 0x42, 0x46, 0x47, 0x4C, 0x4D, 0x4E, 0x4F, 0x50.
 *
 * Arm 0x1C is decoded here (the lofty-castle orbiting mote: a sine/cosine LUT
 * velocity rotated through the source actor's matrix).  It needs its OWN stack
 * vector, declared between arm 0xC's and arm 0x21's -- sharing arm 0x21's costs
 * the whole frame 16 bytes and shifts every sp offset.
...
void func_level_26_80086438(int count, int type, int *pos, int arg3) {
  int v0[3];
  int v1[3];
  int vc[3];
  int v2[3];
  ...
  for (i = 0; i < count; i++) {
    ...
    case 0x1C: {
      int n;
      n = func_8006272C() & 0xFF;
      vc[0] = 0;
      vc[1] = (arg3 * D_8006CC78[n]) >> 12;
      vc[2] = (arg3 * D_8006CBF8[n]) >> 12;
      func_80017048(pos + 8, vc, vc);
      rec = (Emit *)func_80053570(2);
      rec->type = type;
      rec->phase = func_8006272C() & 0xF;
      rec->unk03 = 1;
      func_80017BFC(rec->u.spark.pos, pos + 3);
      rec->u.spark.life = rec->phase * 4;
      rec->u.spark.seed = func_8006272C();
      rec->u.spark.vel[0] = vc[0];
      rec->u.spark.vel[1] = vc[1];
      rec->u.spark.vel[2] = vc[2];
      rec->u.spark.unk1E = (func_8006272C() & 7) - 4;
      rec->u.spark.r = 0x80;
      rec->u.spark.g = 0x80;
      rec->u.spark.b = 0x80;
      rec->u.spark.fx = 0x2E;
