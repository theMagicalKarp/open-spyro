/* func_level_0_80086134 (0x80086134, level_0_artisans_home overlay,
 * 0x12ac bytes).
 *
 * The emit-list ADVANCE pass -- the Artisans-home 1195-insn variant.  Walks
 ...
 *
 * The Artisans-home arm set: the ground classes 0x6 (the orbiting spark) and
 * 0x18 (the fire jet) plus the midpoint families all transfer verbatim from
 * the matched level_6 / level_2 copies.  Three arms are new here:
 *   0xE   a whirlwind whose two spin components decay one step per frame
 *         toward zero and whose r/g/b fade together off the red channel;
 *   0x16  the same decay pair, but the colour ramp is driven by the global
 *         step D_800756CC and the green/blue channels track half of red;
 *   0x1E  a drifting mote that advances its height by the record's own phase
 *         word -- the one arm in this variant that dies on age ALONE, with no
 *         alive-byte test, so it cannot use the shared kill tail.
 *
 * The decay pairs are the shape lever here: `short a; if (a > 0) { a--;
 * HS(n) = a; }` is what produces the original's `lh a; move copy,a; addiu
 * copy,copy,-1` (an `int` local computes in place and loses the copy; a
 * `short` with `HS(n) = a - 1` reloads the field with an `lhu` instead).
 * At 21 arms the allocator spills step*2 to the frame (0x48(sp)) and keeps
 * only step*4 in a callee-saved register -- that falls out of the arm count,
 * not out of the source, as does the 0xA0 frame.
 *
 * Verified byte-identical inside the relinked level_0_artisans_home.ovl.
 */
...
void func_level_0_80086134(int step) {
