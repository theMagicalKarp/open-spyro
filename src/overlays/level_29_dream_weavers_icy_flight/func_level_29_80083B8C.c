/* func_level_29_80083B8C (0x80083B8C, level_29_dream_weavers_icy_flight
 * overlay, 0xc98 bytes).
 *
 ...
 * (bar branch displacements) to func_level_5_80084128.
 *
...
void func_level_29_80083B8C(int count, int type, int *pos, int arg3) {
  int v0[3];
  int v1[3];
  int v2[3];
  int v3[3];
  ...
  for (i = 0; i < count; i++) {
    ...
    case 0x1B: {
      int w;
      int q;
      rec = (Emit *)func_80053570(6);
      rec->type = type;
      rec->phase = 0;
      rec->unk03 = 1;
      func_80017BFC(rec->u.spark.pos, pos);
      rec->u.spark.life = ((arg3 >> 16) * 6) + 0xC;
      w = D_80078A66 + (func_8006272C() & 7) + 0x1D;
      q = (arg3 & 0xFFFF) * 8;
      rec->u.spark.r = q;
      rec->u.spark.g = q;
      rec->u.spark.b = q;
      rec->u.spark.seed = w;
      if ((func_8006272C() & 7) == 0) {
        rec->u.spark.r = rec->u.spark.r + 0x10;
        rec->u.spark.g = rec->u.spark.g + 0x10;
        rec->u.spark.b = rec->u.spark.b + 0x10;
      }
      rec->u.spark.fx = 0x2E;
      rec->u.spark.unk11 = 4;
      rec->u.spark.unk10 = 0;
