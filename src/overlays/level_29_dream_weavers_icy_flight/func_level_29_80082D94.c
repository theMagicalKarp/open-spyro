/* func_level_29_80082D94 (0x80082D94, level_29_dream_weavers_icy_flight
 * overlay, 0xdf8 bytes).
 *
 * The emit-list ADVANCE pass -- the sibling of func_level_29_80082028 (which
 * pushes records).  Walks the 0x20-byte record stream at D_80075824 and ages
 * one record per iteration: integrates its position, fades its colour, ticks
 * its life counter and, when the record has expired (or its alive byte has
 * been cleared), hands it back with func_80053608().  The walk stops at the
 ...
 * induction variable: the terminator test then sits at offset 0 (not a giv
 * at all), and the base falls to whichever address the last arm touches.
 *
 ...
 *
 * The flight-overlay 894-insn variant: the same 16 arms as the 790-insn copy
 * in level_11 / level_17 / level_23, plus the two flight-only classes 0xD
 * (the tinted contrail, whose colour ramp is a three-way age split) and 0x13
 * (the smoke puff, which spins down its angular velocity at +0x18 and fades
 * to a grey ramp).  Word-identical (bar branch displacements) to the copy in
 * level_5_artisans_sunny_flight.
 *
 * Verified byte-identical inside the relinked
 * level_29_dream_weavers_icy_flight.ovl.
 */
...
void func_level_29_80082D94(int step) {
