...
/* Type 0xA3: a bobbing walker; when hit it pays out, releases an 0xE8
 * helper and runs off. */
typedef struct BobberState {
  int reward;  /* 0x00 */
  int tally;   /* 0x04 */
  int phase;   /* 0x08 0..479 */
  int unk0C[2];
  int baseZ;   /* 0x14 */
  int unk18;   /* 0x18 passed by address to func_80039910 */
  int heading; /* 0x1C */
  int unk20;   /* 0x20 passed by address to func_80039910 */
} BobberState;

/* Type 0xE8: the helper; spins and runs until it gets away or times out. */
...
extern void func_8002B390(int a, int b, int c);

void func_level_29_8007CFB4(void) {
  Actor **actorList;
  Actor *actor;
  SpyroObj *spy;
  int one;
  ...
  while (actor = *actorList++) {
    ...
    case 0xA3: {
      BobberState *st = actor->state;

      if ((actor->flags & 0x10000) && actor->unk48 == 0) {
        Actor *reward;
      ...
      case 0:
        ...
        actor->posZ =
            st->baseZ + (D_8006CC78[(st->phase << 8) / 480] >> 1) + 0x800;
        break;
      case 1:
        if (D_80075794 != 0) {
          actor->unk48 = 2;
      ...
      case 2:
        ...
        if (func_80039910(actor, &st->unk20, st->heading, &st->unk18, 0xC,
                          0xC) == 3) {
    ...
    case 0xE8: {
      ...
      actor->unk46 += 8;
      if (func_80039910(actor, &st->unk08, st->heading, &st->unk04, 0xC,
                        0xC) == 3) {
        func_80052568(actor);
        continue;
      }
