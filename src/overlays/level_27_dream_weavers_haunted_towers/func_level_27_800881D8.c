/* func_level_27_800881D8 (0x800881D8, level_27_dream_weavers_haunted_towers
...
typedef struct Emit {   /* one 0x20-byte emit-list record */
  ...
  union {
    ...
    struct {                     /* class 3 -- owner-tethered spark */
      unsigned short pos[3];     /* 0x04 */
      unsigned char life;        /* 0x0A */
      unsigned char seed;        /* 0x0B */
      unsigned char r;           /* 0x0C */
      ...
      unsigned char unk11;       /* 0x11 */
      short pad[3];              /* 0x12 */
      unsigned char owner;       /* 0x18 actor index into the pool */
      unsigned char unk19;       /* 0x19 */
      unsigned char unk1A;       /* 0x1A */
      unsigned char unk1B;       /* 0x1B */
      short unk1C;               /* 0x1C */
      short unk1E;               /* 0x1E */
    } tether;
...
void func_level_27_800881D8(int count, int type, int *pos, int arg3) {
  int v0[3];
  int v1[3];
  int v2[3];
  int v3[3];
  ...
  for (i = 0; i < count; i++) {
    ...
    case 0x1D: {
      ...
      int t1, t2, t3;
      int u1, u2, u3;
      owner = ((Actor *)arg3)->owner;
      rec = (Emit *)func_80053570(3);
      rec->type = type;
      rec->phase = func_8006272C() & 7;
      rec->unk03 = 1;
      func_80017BFC(rec->u.tether.pos, pos);
      t1 = func_8006272C() & 0x7E;
      u1 = rec->u.tether.pos[0] - 0x3F;
      rec->u.tether.pos[0] = u1 + t1;
      t2 = func_8006272C() & 0x7E;
      u2 = rec->u.tether.pos[1] - 0x3F;
      rec->u.tether.pos[1] = u2 + t2;
      t3 = func_8006272C() & 0x7E;
      u3 = rec->u.tether.pos[2] - 0x3F;
      rec->u.tether.pos[2] = u3 + t3;
      rec->u.tether.owner = (Actor *)arg3 - D_80075828;
      rec->u.tether.unk19 = owner[15];
      rec->u.tether.unk1A = 0;
      rec->u.tether.unk1E = 0;
      rec->u.tether.life = (func_8006272C() & 0xF) + 0x30;
      rec->u.tether.seed = (func_8006272C() & 7) + 0x1E;
      rec->u.tether.fx = 0x2E;
      rec->u.tether.r = 0;
      rec->u.tether.g = 0x80;
      rec->u.tether.b = 0;
      rec->u.tether.unk11 = 4;
      rec->u.tether.unk10 = 0;
      break;
    }
    case 0x1E: {
      rec = (Emit *)func_80053570(2);
