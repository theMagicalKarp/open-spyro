/* func_level_6_8008AE28 (0x8008AE28, level_6_peace_keepers_home overlay,
 * 0x11c8 bytes).
 *
 * The emit-list ADVANCE pass -- the Peace-Keepers-home 1138-insn variant.
 ...
 *
 * A hub mix: the flight contrail 0xD, the ground fire jet 0x18, the drifting
 * mote 0x4A and the midpoint class 0x50 all transfer verbatim from the
 * matched level_5 / level_2 / level_24 copies; only 0x17 -- the banner mote,
 * whose blue channel is driven by the record's own phase toggle instead of
 * the step count -- is new here.
 *
 * Verified byte-identical inside the relinked level_6_peace_keepers_home.ovl.
 */
...
void func_level_6_8008AE28(int step) {
  ...
  do {
    ...
    case 0x17:
      /* The peace-keepers home banner mote: its blue channel is driven by
       * the record's own phase toggle rather than by the step count. */
      func_80017C84(POS, POS, VEL);
      rec[0xB] += rec[0x1E];
      rec[0xA] += step * 2;
      rec[0xC] -= step * 2;
