/* func_level_9_80085184 (0x80085184, level_9_peace_keepers_ice_cavern
 ...
 * This variant's classes: 0x0, 0x1, 0x2, 0x9, 0xA, 0xC, 0xF, 0x15, 0x18, 0x21, 0x42, 0x46, 0x47, 0x4B, 0x4C, 0x4D, 0x4E, 0x4F, 0x50.
 * PLACEHOLDER *
 * Arm 0x18 is the family's per-variant knob: `fx = 0x2E` must be a plain
 * literal, `unk11`/`unk18` written after it, and the 0x30 carrier must be
 * MULTI-SET or `li v0,48` is birthing-boosted and swaps with `sll v1,i,4`.
 * Here that carrier is `v`.
 */
...
void func_level_9_80085184(int count, int type, int *pos, int arg3) {
  int v0[3];
  int v1[3];
  int v2[3];
  int v3[3];
  ...
  for (i = 0; i < count; i++) {
    ...
    case 0xF: {
      ...
      rec->unk03 = 1;
      func_80017BFC(rec->u.spark.pos, pos);
      rec->u.spark.vel[0] = (func_8006272C() & 0x1E) - 0xF;
      rec->u.spark.vel[1] = (func_8006272C() & 0x1E) - 0xF;
      rec->u.spark.vel[2] = (func_8006272C() & 0x1E) - 0xF;
      rec->u.spark.pos[0] = rec->u.spark.pos[0] + (unsigned short)rec->u.spark.vel[0];
      rec->u.spark.pos[1] = rec->u.spark.pos[1] + (unsigned short)rec->u.spark.vel[1];
      rec->u.spark.pos[2] = rec->u.spark.pos[2] + (unsigned short)rec->u.spark.vel[2];
      rec->u.spark.life = (func_8006272C() & 7) + 0xC;
      rec->u.spark.seed = (func_8006272C() & 7) + 0xA;
      rec->u.spark.r = 0x80;
      rec->u.spark.g = 0x80;
      rec->u.spark.b = 0x80;
      rec->u.spark.fx = 0x2C;
    ...
    case 0x4B: {
      ...
      rec->unk03 = 1;
      func_80017BFC(rec->u.spark.pos, pos);
      rec->u.spark.pos[0] = rec->u.spark.pos[0] - (0x1F - (int)(func_8006272C() & 0x3E));
      rec->u.spark.pos[1] = rec->u.spark.pos[1] - (0x1F - (int)(func_8006272C() & 0x3E));
      rec->u.spark.pos[2] = rec->u.spark.pos[2] - (0x1F - (int)(func_8006272C() & 0x3E));
      rec->u.spark.vel[0] = 0;
      rec->u.spark.vel[1] = 0;
      rec->u.spark.vel[2] = 0;
      rec->u.spark.r = 0xFF;
