/* func_level_14_8008883C (0x8008883C, level_14_magic_crafters_high_caves
 * overlay, 0xC18 bytes).
 *
...
/* Homing-shot state block (type 0x130). */
...
extern void func_800176F0(int *vec);                     /* ZeroVector */
...
Actor *func_level_14_8008883C(int type, Actor *parent) {
  Actor *rec;
  int owner;
  ...
  case 0x127: {
    ...
    func_8003A720(rec);
    rec->posX = parent->posX + (D_8006CC78[parent->unk46] >> 1);
    rec->posY = parent->posY + (D_8006CBF8[parent->unk46] >> 1);
    rec->posZ = parent->posZ + 0x300;
    dx = D_80078A58[0] - rec->posX;
    turn = (func_80016AB4(dx, D_80078A5C - rec->posY, 0) - parent->unk46) & 0xFF;
    if (turn > 0x80) {
      turn -= 0x100;
    }
    if (turn < -0x20) {
  ...
  case 0x13E: {
    ...
    func_8003A720(rec);
    func_80017700(&rec->posX, &parent->posX);
    st[0] = (func_8006272C() & 0x7E) - 0x3F;
    st[1] = (func_8006272C() & 0x7E) - 0x3F;
    st[2] = (func_8006272C() & 0x7E) - 0x10;
    st[3] = (func_8006272C() & 0x1F) + 0x20;
    func_800526A8(rec);
    break;
  }
  case 0x188:
